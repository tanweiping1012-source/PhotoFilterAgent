...
| `v3` | `e447f8f` | **六维 rubric + 双向盲比 + 独立盲审**。约 6000 行 TypeScript、18 个工具。⚠️ 未验收的工作现场 | 3/20 p=0.130 ⚠️ | **997 次** | [V3](V3-agent-rubric.md) · [原始设计稿](V3-original-spec-by-codex.md) |
...
**v1 没有 tag** —— 纯本地技术指标那一版没有独立的提交边界，它一直演化成了 v2。
`engine/` 目录里那套 Swift 分析引擎就是它，**至今仍在用**（v4.2 的闭眼资格门就来自它）。

**v3 是补档的。** 它的代码在另一份工作副本上、从未推送过 ——
也就是说，这个项目最贵的一版（997 次付费调用）**此前只存在于一台机器上，没有任何备份**。
2026-08-31 从那份工作副本拉取 `e447f8f` 归档为 `v3-archive` 分支并打 tag。

⚠️ 那是 Codex 2026-08-28 的**工作现场**，未通过他自己在
`README.CANDIDATE.md` 里定的 16 条发布门槛（只勾了 1 条）。
`v3` 这个 tag 的意义是「保住这份代码」，不是「这一版可以用」。
归档前查过：0 张照片、0 处凭据、2 处路径（都在说明文字里）。
...
| 眼部判定搬到原图（含被否决的按组归一） | [V4.3-eyes-on-full-res.md](V4.3-eyes-on-full-res.md) |
| 三阶段链路与可插拔裁判 | [V4.4-three-stage-pipeline.md](V4.4-three-stage-pipeline.md) |
...
v4.3 发现**最强的那个信号一直是瞎的** —— 它跑在缩略图上，
三分之二的照片压根没读数，而用户 27/35 次的判断依据正是它。
v4.4 把三段真正接起来 —— 组内选优以前是个空壳，
而补上它带来的收益**与裁判是谁无关**。
