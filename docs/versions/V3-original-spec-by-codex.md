---
> **这是 v3 的原始设计稿，由 Codex 在 2026-08-27 撰写，原文件名 `README.CANDIDATE.md`（707 行）。**
>
> 保留原文是因为它记录了 v3 **当时**的完整设计意图和自我要求 —— 包括那份从未被勾满的
> 「正式发布门槛」清单。它写于验收结果出来之前，所以里面对能力的描述是**设计目标**，
> 不是实测结论。
>
> 实测结果、事后诊断和「哪些设计被 v4 继承 / 换掉」，见 [V3 版本记录](V3-agent-rubric.md)。
---
...
- 旧 289 张候选运行完成了全池 low、selector high/AB/BA，并在一次有效 audit FAIL 后依据审计反例
  重建出新的 exact-20；新名单的第二次 audit 已完成 selected high 20/20、remaining low 268/269。
  后续确认人工 pick 的 20 张原图当时已从这个目录移走，因此 ground truth 从未进入该候选池；这轮
  只能作为断点恢复、FAIL 反馈重建和 remaining-only 的过程证据，不能用于重合率验收。
- 另一轮完整 309 张运行已经完成 MiniMax-M3 的全池 baseline 309/309、selector high 60/60 和
  6 组/12 legs AB/BA，冻结了新的 exact-20；独立 audit 当前为 selected high 20/20、remaining low
  94/289，总覆盖 114/309，Stage C/D 尚未开始。这是本轮可继续使用的真实付费 checkpoint。
- 已在同一个 309 张候选根目录中增加只读硬链接 oracle 子树，并由 preset 在枚举、fingerprint 和
  匿名 ID 之前显式排除；原照片没有被移动、删除、覆盖或修改。排除后本地重扫仍为 309 张、14 个
  families，dataset fingerprint 与已有 checkpoint 完全一致，因此恢复后不应重新支付 baseline。
- 隔离本地预检实测 `candidate_count=309`、`oracle_count=20`、
  `oracle_content_present_count=20`、`candidate_oracle_coverage=1.0`、K=20 对齐，数据集有效。该预检
  只读取内容并输出汇总数字，没有向 selector 暴露文件名、哈希或具体命中关系。
- 额度耗尽时，旧 DSH 把明确的 `429 / 2056 Token Plan` 错分为瞬时 `RATE_LIMIT` 并在同一请求链
  重试 5 次。本机 Harness 已补充 terminal quota 分类：155 个定向测试通过；重启后真实请求得到
  `code=QUOTA`，只有一次请求、没有 `llm/retry`、没有工具调用或 checkpoint 改写。
- 当前离线回归已覆盖 audit FAIL 反馈重建、上游修正后的 bounded plan 重算、同 hash 防循环、验收
  数据集有效性、oracle 隔离、可恢复调用账本和跨批次失败明细；当前完整回归为 108/108 通过。

因此目前可以确认“309 张已完成 selector 评分并进入独立审计、验收数据集有效、额度熔断不再空转”，
但 audit 仍只有 114/309，不能说方案通过最终验收，也不能生成 receipt 或发布重合率。最近两次有界
续跑新增 attempted=64、succeeded=43、failed=21、unresolved=0；累计账本为 attempted=135、
succeeded=114、failed=21、unresolved=0、unique cached assets=114。随后一次新 turn 在调用领域工具前
即收到 terminal `429 QUOTA / 2056`，只有一次父模型请求且 checkpoint 未变化。
...
3. 在现有 309 张父会话中选择原 checkpoint 相同的 provider/model，运行一次 `analyze_folder`：输入根
   为已含显式排除 oracle 子树的 309 张目录，people target=20、scenery target=0、people_only、无
   limit。该恢复验证已经完成：fingerprint、309/309 baseline、exact-20 selection hash 和 audit
   checkpoint 均未漂移，也没有重新支付 baseline。
4. 使用恢复 draft 的五项冻结输入只调用一次 `independent_evaluator`；当前从 114/309 继续，后续 turn
   只补 remaining。遇到
...
8. 完成安全、缓存、模型路由和网络端点复核：全链路只能出现当前会话所选 provider endpoint，不能
   出现隐藏 fallback。
9. 核对 audit 新增 attempted/succeeded/failed/unresolved 与唯一缓存统计；对已有 71 次成功的旧阶段，
