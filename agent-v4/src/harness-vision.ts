 ...
 * 也不能把一整批照片发出去。
 *
 * 40 的来历：锚点 3 组共 14 张照片、考题 2 张，每张照片发**两幅图**
 * （整幅 + 高清人脸）= 32 幅，留一点余量。
 *
 * 为什么锚点也要发人脸特写：原本只发 512px 整幅，实测那上面人脸只有
 * **24 像素**，而锚点的文字在说「4、5、6 闭眼」「1、3 眼神不自然」——
 * 模型根本验证不了，只能把那段话当口号背下来。考题图早就修成
 * 「整幅 + 人脸特写」了，锚点这一路一直没跟上。
 *
 * 40 仍远小于任何一批真实照片的规模，护栏的作用不变。
 */
export const MAX_JPEGS_PER_CALL = 40
